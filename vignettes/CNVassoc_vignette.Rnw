...
Let's load the data. There are 2 data frames, one for cases and the other for controls. For this example we have selected 50 of the 100 imputed SNPS.
<<echo=TRUE>>=
data(SNPTEST)
dim(cases)
dim(controls)
...
Now \texttt{probs} is a list of 50 components, each one containing the probability matrix of each SNP, and the first 500 rows of each matrix
