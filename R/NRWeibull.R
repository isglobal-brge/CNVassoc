NRWeibull <- function(y, cens, X, w, beta, 
        alpha, variant, tol = 10^-6, max.iter = 1000, 
        verbose = FALSE) {
        if (missing(variant)) 
        ...
        if (max.iter <= 0) {
            derivatives <- hessianWeibull(beta = beta, 
                alpha = alpha, y = y, cens = cens, 
                w = w, X = X, variant = variant)  #
            S <- derivatives$S
            H <- derivatives$H
        }
        iter <- 1
        ...
        while (error > tol & iter < max.iter) {
            ...
            beta.old <- beta
            alpha.old <- alpha
            beta.oldv <- matrix2vector(beta.old, 
                variant)
            theta.oldv <- c(beta.oldv, alpha)
            thetav <- as.vector(theta.oldv - 
                qr.solve(H) %*% S)
            betav <- thetav[seq_len(numbetes)]
            beta <- vector2matrix(betav, variant, 
                J)
        ...
        return(list(beta = beta, alpha = alpha, 
            variant = variant, score = S, hessian = H))
}
