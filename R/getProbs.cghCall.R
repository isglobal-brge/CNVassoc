getProbs.cghCall <- function(x) {
        ...
        } else stop("CGHbase is not available")
        
        colnam <- c(colnames(Probloss), colnames(Probnorm), 
            colnames(Probgain))
        if (is.null(Probamp)) {
            allprob <- c()
            ncl <- ncol(Probnorm)
            for (i in seq_len(ncl)) {
                Probsall <- cbind(Probloss[, 
                    i], Probnorm[, i], Probgain[, 
                    i])
                colnames(Probsall) <- c(colnam[i], 
                    colnam[ncl + i], colnam[2 * 
                    ncl + i])
        ...
        probs <- data.frame(Clone, Chromo, BPstart, 
            BPend, allprob2)
        probs
}
